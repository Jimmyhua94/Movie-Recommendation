...
{
	vector<vector<int>> genres(6, vector<int>(3, 5));
	for (map<int, int>::iterator it = users->at(person).movieRating.begin(); it != users->at(person).movieRating.end(); it++) {
	...
	for (int i = 0; i < 6; i++){
		users->at(person).genreRating[i] = genres[i][0]/genres[i][2];
		users->at(person).pref[i] = users->at(person).genreRating[i] / (genres[i][1] / genres[i][2]);
	}
}